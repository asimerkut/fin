...
FROM hop_finansal_hareket_detay t1
            INNER JOIN hop_finansal_hareket t2 ON t2.id = t1.finansal_hareket_id
            INNER JOIN hop_dosya t3 ON t3.id = t2.dosya_id
            INNER JOIN hop_dosya_borc t4 ON  t4.id = t1.dosya_borc_id
            LEFT OUTER JOIN hop_dosya_borc_kalem t5 ON  t5.id = t1.dosya_borc_kalem_id
            LEFT OUTER JOIN hop_borc t6 ON t6.id =  t5.borc_id
            LEFT OUTER JOIN hop_masraf t7 ON t7.id =  t5.masraf_id
